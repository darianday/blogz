...
def blog():
    ...
    if blog_id == None:
        ...
        return render_template('blog.html', blog_posts = blog_posts, title = 'build-a-blog')
    else:
        blog_id = int(blog_id)
        post = Blog.query.get(blog_id)
        return render_template('blog_entry.html', post = post, title = 'Blog Entry')
...
def new_post():
    if request.method == 'POST' :
        ...
        if not blog_title:
            title_error = "Please enter the title of the blog"
        if not blog_body: 
